在程序运行期间，Java 运行时系统始终为所有对象维护一个运行时类型标识，指向该对象所属类型的 Class 对象。
```java
User user = new User();
...
User 类定义了 user 对象有哪些属性和行为，user 对象为具体的实例。Class 类定义了所有与类有关的信息，userClass 对象为具体的实例，可以通过 userClass 获取 User 类定义的内容。

在程序启动时，包含 main 方法的类被加载。它会加载所有需要的类，这些被加载的类又会加载它们需要的类，以此类推。所有的类都是在对其第一次使用时，动态加载到 JVM 中的。
...
public class ReflectiveTest {  
    ...
    public static void main(String[] args) throws ReflectiveOperationException{  
        ...
        // 获取类的修饰符，例如 public final        String modifiers = Modifier.toString(clz.getModifiers());  
...
使用 arthas 调试输出以经加载的类，查看 App、TestLoadOne 和 TestLoadTwo 的加载时机。
```
java -jar arthas-boot.jar
> sc *otto*
```

结论：程序启动后默认会加载 App 类，输入 one 之后加载 TestLoadOne 类，输出 two 之后加载 TestLoadTwo 类。