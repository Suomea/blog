...
- `java.lang.Class` 类实现 `Type` 接口。
- `java.lang.reflect.TypeVariable` 表示**类型变量**，它用来反映在 JVM 编译该泛型前的信息。例如 `List<T>` 其中 `T` 就是类型变量。
- `java.lang.reflect.ParameterizedType` 表示**参数化类型**，例如 `List<T>`、`Map<K, V>` 这种带有泛型的类型。
