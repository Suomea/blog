...
启动选项
--web.listen-address 配置 prometheus webui 监听的端口号
...
--web.enable-lifecycle 允许访问接口热加载，`curl -X POST http://127.0.0.1:9090/-/reload` 进程不中断重载配置和报警配置文件。

### 生产环境部署
生产环境推荐的部署方式：
1. Prometheus 仅监听本地端口。
2. 创建单独的用户，作为 Prometheus 进程的启动用户。
3. Prometheus 使用 systemd 托管进程，日志由 journald 统一接管。
4. Nginx 代理 Prometheus Web 端口，提供 SSL 和 BasicAuth 访问。

创建用户
```
useradd -r -M --shell /sbin/nologin prometheus
```

创建目录结构
```
mkdir -p /srv/prometheus/{bin,data,rules}
```

移动文件
```
tar -zxvf prometheus-3.9.1.linux-amd64.tar.gz 
cp prometheus-3.9.1.linux-amd64/prometheus prometheus-3.9.1.linux-amd64/promtool /srv/prometheus/bin/
cp prometheus-3.9.1.linux-amd64/prometheus.yml /srv/prometheus/
```
...
ExecStart=/srv/prometheus/bin/prometheus \
...
```
    server {
        ...
        location / {
            proxy_pass http://127.0.0.1:9090;  # Prometheus 默认端口
...
```
Basic Auth 和自签名证书参考：[Nginx 配置 Basic Auth](../Nginx/Nginx 配置 Basic Auth) 和 [Nginx 使用签名证书](../Nginx/Nginx 使用自签名证书)。
