...
### 配置信息
我准备的安装环境为 Windows Hyper-V 管理的三台 Linux 虚拟机上，虚拟机均为 Debian 系统。具体配置如下表格，如无特别说明，后续所有的安装配置
在三台机器上都要操作。

| hostname | ip | RAM | CPUs | 描述 |
| :-- | :-- | :-- | :-- | :-- |
...
## 安装容器运行时（CRI）
每个节点都要安装容器运行时，这样 Pod 才能在节点上运行，所以下面的操作需要在每个机器上都要执行。
...
临时关闭 swap 分区可以使用 `swapoff -a` 命令；然后注释掉 `/etc/fstab` 文件中配置开机挂载 swap 分区的行。
...
然后编辑 ~/.bashrc，配置相关命令的自动补全，然后使用 `source ~/.bashrc` 使配置生效。
```text
...
可以使用 `kubeadm version` 命令查看即将要安装的 K8S 的版本；用 `kubeadm config images list` 命令查看初始化集群需要的镜像；
...
localAPIEndpoint:
  # 修改为 control-plane 所在机器的 IP
  advertiseAddress: 192.168.31.11
  bindPort: 6443
nodeRegistration:
  ···
  # 修改为 control-plane 所在机器的 hostname
  name: suomea01
  taints: null
---
apiServer:
